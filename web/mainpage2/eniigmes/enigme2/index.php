<?php
  ...
  $conditionDone = False;
  $request=$bdd->prepare('UPDATE participant SET enigme2 = :enigme2 WHERE pseudo = :pseudo');
  ...
  <div class="container-margin">
    <main class="txt">
                <div class="icn">
                 ...
                 <h1>Enigme 2 :<span></span></h1>
                </div>
                <?php 

                $requestCheck = $bdd->query('SELECT pseudo, enigme2 FROM participant');
                while($donnees=$requestCheck->fetch()) {
                      if($donnees["pseudo"]==strtolower(htmlspecialchars($_SESSION['pseudo']))&& $donnees["enigme2"]==1){
                        $conditionDone = True; 
                      }
                    }
